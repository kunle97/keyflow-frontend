...
import { Input, Button, Typography, Alert } from "@mui/material";
import { Link } from "react-router-dom";
import ProgressModal from "../UIComponents/Modals/ProgressModal";
import { set, useForm } from "react-hook-form";
import { validationMessageStyle } from "../../../constants";
import { getStaffEmails, getStaffUsernames } from "../../../api/api";
import {
  triggerValidation,
  validateForm,
...
const StaffLogin = () => {
  ...
  const [staffEmails, setStaffEmails] = useState([]); //TODO: get usernames from db and set here
  const [staffUsernames, setStaffUsernames] = useState([]); //TODO: get usernames from db and set here
  ...
  const onSubmit = async (e) => {
    ...
    if (response.token) {
      ...
      setRedirectURL("/dashboard/staff");
  ...
  useEffect(() => {
    getStaffEmails().then((res) => {
      if (res) {
        setStaffEmails(res);
      }
    });
    getStaffUsernames().then((res) => {
      if (res) {
        setStaffUsernames(res);
      }
    });
  }, []);
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(rgba(0,0,0,0.48) 0%, rgba(0,0,0,0.1) 99%),url('/assets/img/staff-login-page-banner.jpg')",
